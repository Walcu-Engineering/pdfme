...
import { cmyk, degrees, degreesToRadians, rgb, Color } from '@walcu-engineering/pdfme-pdf-lib';
import { Schema, mm2pt, Mode, isHexValid, ColorType } from '@walcu-engineering/pdfme-common';
...
export const readFile = (input: File | FileList | null): Promise<string | ArrayBuffer> =>
  new Promise((resolve, reject) => {
    ...
    fileReader.onerror = () => {
      reject(new Error('[@walcu-engineering/pdfme-schemas] File reading failed'));
    };
    ...
    if (file) {
      ...
      reject(new Error('[@walcu-engineering/pdfme-schemas] No files provided'));
    }
  });
