import { PDFDocument, RotationTypes } from '@walcu-engineering/pdfme-pdf-lib';

const merge = async (pdfs: (ArrayBuffer | Uint8Array)[]): Promise<Uint8Array> => {
  if (!pdfs.length) {
    throw new Error('[@walcu-engineering/pdfme-manipulator] At least one PDF is required for merging');
  }
...
const split = async (
  ...
  if (!ranges.length) {
    throw new Error('[@walcu-engineering/pdfme-manipulator] At least one range is required for splitting');
  }
  ...
  for (const { start = 0, end = numPages - 1 } of ranges) {
    if (start < 0 || end >= numPages || start > end) {
      throw new Error(
        `[@walcu-engineering/pdfme-manipulator] Invalid range: start=${start}, end=${end}, total pages=${numPages}`,
      );
    }
...
const remove = async (pdf: ArrayBuffer | Uint8Array, pages: number[]): Promise<Uint8Array> => {
  if (!pages.length) {
    throw new Error('[@walcu-engineering/pdfme-manipulator] At least one page number is required for removal');
  }
  ...
  if (pages.some((page) => page < 0 || page >= numPages)) {
    throw new Error(
      `[@walcu-engineering/pdfme-manipulator] Invalid page number: pages must be between 0 and ${numPages - 1}`,
    );
  }
...
const insert = async (
  ...
  for (let i = 0; i < inserts.length; i++) {
    ...
    if (actualPos < 0 || actualPos > numPages) {
      throw new Error(`[@walcu-engineering/pdfme-manipulator] Invalid position: must be between 0 and ${numPages}`);
    }
...
const rotate = async (
  ...
  if (!Number.isInteger(degrees) || degrees % 90 !== 0) {
    throw new Error('[@walcu-engineering/pdfme-manipulator] Rotation degrees must be a multiple of 90');
  }
  ...
  if (!pages.length) {
    throw new Error('[@walcu-engineering/pdfme-manipulator] PDF has no pages to rotate');
  }
  ...
  if (pageNumbers) {
    if (pageNumbers.some((page) => page < 0 || page >= pages.length)) {
      throw new Error(
        `[@walcu-engineering/pdfme-manipulator] Invalid page number: pages must be between 0 and ${pages.length - 1}`,
      );
    }
  }
...
const move = async (
  ...
  if (from < 0 || from >= currentPageCount || to < 0 || to >= currentPageCount) {
    throw new Error(
      `[@walcu-engineering/pdfme-manipulator] Invalid page number: from=${from}, to=${to}, total pages=${currentPageCount}`,
    );
  }
...
const organize = async (
  ...
  if (!actions.length) {
    throw new Error('[@walcu-engineering/pdfme-manipulator] At least one action is required');
  }
  ...
  for (const action of actions) {
    ...
    switch (action.type) {
      ...
      default:
        throw new Error(
          `[@walcu-engineering/pdfme-manipulator] Unknown action type: ${(action as { type: string }).type}`,
        );
    }
  }
