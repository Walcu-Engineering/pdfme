...
\*どのパッケージを使用する場合でも、`@walcu-engineering/pdfme-common`をインストールする必要があります。
...
テンプレート型は`@walcu-engineering/pdfme-generator`または`@walcu-engineering/pdfme-ui`の両方からインポートできます。テンプレートはあらゆる場所で使用されます。
...
**schemas**はデフォルトではテキストのみ使用できますが、`@walcu-engineering/pdfme-schemas`パッケージからプラグインとして画像やQRコードなどの様々なバーコードを読み込むことができます。  
...
デフォルトでは、例は多くの場合`text`スキーマタイプの使用を示しています。ただし、他の組み込みスキーマタイプを使用したり、`@walcu-engineering/pdfme-schemas`パッケージで独自のカスタムスキーマを作成したりすることもできます。

#### ステップ1: `@walcu-engineering/pdfme-schemas`のインストール
